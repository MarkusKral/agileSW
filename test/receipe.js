...
describe("Testing receipes.", function () {
    ...
    // drop the collection and generate 2 new receips
        mongoose.connection.db.dropCollection("receipe", function (err, result) {
            ...
            rec1.save(function (err, receipe) {
                if (err)
                    console.log(err);
            });
            rec2.save(function (err, receipe) {
                if (err)
                    console.log(err);
            });
        });
        ...
        it("should return only one receipe", function (done) {
            chai.request(server)
                ...
                .end(function (err, res) {
                    ...
                    expect(res.body).to.deep.include({name: "test-receipe"});
                    done();
                });
        });
        ...
        it("search with invalid id", function (done) {
            chai.request(server)
                ...
                .end(function (err, res) {
                    expect(res).to.have.status(200);
                    expect(res.body).to.be.a("Object");
                    expect(res.body).to.deep.include({"name": "CastError"});
                    done();
                });
        });
    });
            ...
            it("Update testreceipe", function (done) {
                authenticatedUser
                    ...
                    .end(function (err, res) {
                        expect(res).to.have.status(200);
                        expect(res.body).to.be.a("Object");
                        expect(res.body).to.deep.include({_id: id});
                        expect(res.body).to.deep.include({name: "updated"});
                        expect(res.body.ingredients).to.deep.include({flour: 2});
                        done();
                    });
            });
            ...
            it("Delete testreceipe", function (done) {
                authenticatedUser
                    ...
                    .end(function (err, res) {
                        expect(res).to.have.status(200);
                        expect(res.body).to.be.a("Object");
                        expect(res.body).to.deep.include({message: "Receipe successfully deleted"});
                        done();
                    });
            });
            ...
            it("Create a receipe", function (done) {
                authenticatedUser
                    ...
                    .end(function (err, res) {
                        ...
                        expect(res.body).to.be.a("Object");
                        expect(res.body).to.deep.include({name: "updated"});
                        expect(res.body).to.deep.include({ingredients: {flour: 2}});
                        done();
                    });
            });
